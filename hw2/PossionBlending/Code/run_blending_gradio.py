...
def create_mask_from_points(points, img_h, img_w):
    ...
    if len(points) > 0:
        # fillPoly 接收一个点集的列表，这里 255 表示白色（内部）
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    # 2. 计算前景和混合图的拉普拉斯响应
    # padding=1 确保输出尺寸与输入一致 [B, 3, H, W]
    fg_lap = F.conv2d(foreground_img, kernel, padding=1, groups=3)
    blended_lap = F.conv2d(blended_img, kernel, padding=1, groups=3)

    # 3. 维度对齐（核心修改点）
    # 如果前景图和背景图尺寸不一致，将前景梯度图缩放到背景图的大小
    if fg_lap.shape != blended_lap.shape:
        fg_lap = F.interpolate(fg_lap, size=(blended_lap.shape[2], blended_lap.shape[3]), mode='bilinear', align_corners=False)
    
    # 4. 只计算背景掩码区域内的 Loss
    # 将 background_mask 扩展到 3 通道以便相乘
    mask = background_mask.expand(-1, 3, -1, -1)
    
    # 计算 MSE 损失：只对比被掩码覆盖的区域
    loss = F.mse_loss(blended_lap * mask, fg_lap * mask)
    return loss
