...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 3000):
        ...
        # Sample a fixed subset when COLMAP returns many points. The original
        # scaffold used PyTorch3D farthest-point sampling here; this keeps the
        # dependency footprint small while preserving deterministic training.
        if maximum_pts_num is not None and self.points3D_xyz.shape[0] > maximum_pts_num:
            generator = torch.Generator().manual_seed(0)
            indices = torch.randperm(self.points3D_xyz.shape[0], generator=generator)[:maximum_pts_num]
