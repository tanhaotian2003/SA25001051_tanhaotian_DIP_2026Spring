...
class GaussianTrainer:
    def __init__(
        ...
        self.loss_log_path = Path(config.checkpoint_dir) / "loss_log.csv"
        if not self.loss_log_path.exists():
            self.loss_log_path.write_text("epoch,loss\n")
    ...
    def train(self, train_loader: DataLoader):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            with self.loss_log_path.open("a") as f:
        ...
        if final_epoch >= 0 and final_epoch % self.config.save_every != 0:
...
def parse_args():
    ...
    parser.add_argument('--skip_video', action='store_true',
                      help='Skip the training-camera comparison video after training')
