...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        exponent = -0.5 * torch.einsum('nhwi,nij,nhwj->nhw', dx, inv_cov, dx)
        norm = 1.0 / (2.0 * torch.pi * torch.sqrt(det))
        gaussian = norm.view(N, 1, 1) * torch.exp(exponent.clamp(min=-80.0, max=0.0))
    ...
    def forward(
        ...
        transmittance = torch.cumprod(
            torch.cat([torch.ones_like(alphas[:1]), one_minus_alpha[:-1]], dim=0),
            dim=0,
        )
